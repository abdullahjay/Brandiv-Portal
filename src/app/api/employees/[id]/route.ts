...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { id } = await params;
    const emp = await editEmployee(id, parsed.data);
    if (!emp) return notFound("Employee not found");
    return ok(emp);
  } catch (err) {
    return serverError(err);
